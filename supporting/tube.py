...
def tube(S_fus, diff_p_fus, w_fus, h_fus, l_nose, l_center, l_tail, l_fus, Nlim, wt_zf, wt_wing, wt_propulsion,
    ...
    h_gf = 2 * 2.5  # m - factor of 2 because there are two bays
    w_gf = 1.750  # m
    ...
    A_side = 1.6 * (
        0.5 * h_fus * l_nose * 1.05 + h_fus * l_center + 0.2 * l_tail * (h_fus + 0.6 * h_fus) / 2 + 0.8 * l_tail * (
            0.6 * h_fus + 1.1 * t_hor) / 2)
    A_top = 1.6 * (
        0.5 * w_fus * l_nose * 1.05 + w_fus * l_center + 0.2 * l_tail * (w_fus + 0.9 * w_fus) / 2 + 0.8 * l_tail * (
