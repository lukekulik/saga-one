...
def lateral_directional(velocity, Cn_Beta, S_gross_w, density, span, I_z, Cn_r, I_x, Cl_p, J_xz, Cl_r, Cl_Beta, Cn_p,
                        Cy_phi, Cy_psi, Cy_Beta, mass):
    ...
    J_xz = np.mean(J_xz)
    Cl_r = np.mean(Cl_r)
    Cl_Beta = np.mean(Cl_Beta)
    Cn_p = np.mean(Cn_p)
    Cy_phi = np.mean(Cy_phi)
    Cy_psi = np.mean(Cy_psi)
    Cy_Beta = np.mean(Cy_Beta)
    mass = np.mean(mass)
    ...
    I = np.array([-Cy_Beta, mass * velocity / S_gross_w / (0.5 * density * velocity ** 2.)])
