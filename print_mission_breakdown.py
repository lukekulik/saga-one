...
def print_mission_breakdown(results, filename='mission_breakdown.dat'):
    ...
    for i, (tag, segment) in enumerate(results.segments.items()):  # loop for all segments
        ...
        HPf = -segment.conditions.frames.inertial.position_vector[-1, 2] / Units.m  # Final segment Altitude   [ft]
        HPi = -segment.conditions.frames.inertial.position_vector[0, 2] / Units.m  # Initial segment Altitude  [ft]
        ...
        Dist = (segment.conditions.frames.inertial.position_vector[-1, 0] -
                segment.conditions.frames.inertial.position_vector[0, 0]) / Units.km  # Distance [nm]
        ...
        KCASi = VCi / Units['m/s']  # Calibrated airspeed [knots]
        ...
        KCASf = VCf / Units['m/s']  # Calibrated airspeed [knots]
        ...
        if i == 0:  # Write header
            fid.write(
                '         FLIGHT PHASE           |   ALTITUDE    |     WEIGHT      |  DIST.  | TIME  |  FUEL  |            SPEED              |\n')
            fid.write(
                '                                | From  |   To  |Initial | Final  |         |       |        |Initial| Final |Initial| Final |\n')
            fid.write(
                '                                |   m   |   m   |   kg   |   kg   |    km   |  min  |   kg   | m/s   | m/s   |  Mach |  Mach |\n')
            fid.write(
                '                                |       |       |        |        |         |       |        |       |       |       |       |\n')
        # Print segment data
        fid.write(
            Segment_str + HPi_str + HPf_str + Wi_str + Wf_str + Dist_str + T_str + Fuel_str + KCASi_str + KCASf_str + Mi_str + Mf_str + '\n')

    # Summary of results [nm]
    TotalFuel = results.segments[0].conditions.weights.total_mass[0] - \
                results.segments[-1].conditions.weights.total_mass[-1]  # [kg]
    TotalTime = (
        results.segments[-1].conditions.frames.inertial.time[-1] - results.segments[0].conditions.frames.inertial.time[
            0])  # [min]
